...
async function callAgentModel(
  ...
  if (configuration.model.startsWith("ollama/")) {
    // ChatOllama currently does not support tool_choice parameter
    model = rawModel.bindTools([...MODEL_TOOLS, infoTool])
  } else {
    model = rawModel.bindTools([...MODEL_TOOLS, infoTool], {
      tool_choice: "any",
    })
...
{presumed_info}`
  ...
  try {
    structuredResponse = await boundModel.invoke(messages)
  } catch (err) {
    // If parsing fails, ask the model to retry with correct JSON
    return {
      messages: [
        new HumanMessage(
          "The previous response could not be parsed as valid JSON. Please output a valid JSON object that matches the required schema."
        ),
      ],
    }
  }
